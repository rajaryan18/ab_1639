            ...
            if(area>300):
                ...
                if len(boxes_ids) > 1:
                    if boxes_ids[0][0] < boxes_ids[1][0]:
                        boxes_ids[0][4] = (2*last_aruco)
                        boxes_ids[1][4] = (2*last_aruco) + 1
                    else:
                        boxes_ids[0][4] = (2*last_aruco) + 1
                        boxes_ids[1][4] = (2*last_aruco)

                for box_id in boxes_ids:
                    x, y, w, h, i = box_id
                    if len(boxes_ids) > 1 and i <= (2*last_aruco)+1:
...
def main(args):
    ...
    rospy.Subscriber("/ebot/camera1/image_raw", Image, aruco_callback)
    try:
        rospy.spin()
